...
public class JsFile {
    ...
    @NoteJsFunction(description = "需要将js文件放置到/sdcard/debugger/js/test.js，且包含一个名为add的函数",
            jsFilepath = @NoteJsFilepath(filepath = "file:///sdcard/debugger/js/test.js"),
    ...
    @NoteJsFunction(description = "需要将js文件放置到assets/js/test.js，且包含一个名为minus的函数",
            jsFilepath = @NoteJsFilepath(filepath = "file:///android_asset/js/test.js"),
            parameters = {@Parameter(parameterClass = Integer.class, parameter = "3"),
                    @Parameter(parameterClass = Integer.class, parameter = "5")},
            resultClass = Integer.class)
